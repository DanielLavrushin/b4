...
func TestApplyMigrations(t *testing.T) {
	...
	t.Run("v51 to v52 renames the legacy filter_quic values to sni", func(t *testing.T) {
		cases := []struct {
			name string
			from string
			want string
		}{
			{name: "disabled", from: "disabled", want: QUICFilterSNI},
			{name: "parse", from: "parse", want: QUICFilterSNI},
			{name: "all is left alone", from: "all", want: QUICFilterAll},
			{name: "sni is left alone", from: QUICFilterSNI, want: QUICFilterSNI},
		}
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				cfg := NewConfig()
				set := NewSetConfig()
				set.UDP.FilterQUIC = tc.from
				cfg.Sets = []*SetConfig{&set}

				if err := migrateV51to52(&cfg, map[string]interface{}{}); err != nil {
					t.Fatalf("migration failed: %v", err)
				}
				if set.UDP.FilterQUIC != tc.want {
					t.Errorf("filter_quic: want %q, got %q", tc.want, set.UDP.FilterQUIC)
				}
			})
		}

		t.Run("a set absent from the file loads on the new default", func(t *testing.T) {
			cfg := NewConfig()
			set := NewSetConfig()
			cfg.Sets = []*SetConfig{&set}
			...
			if set.UDP.FilterQUIC != QUICFilterSNI {
				t.Errorf("filter_quic: want %q, got %q", QUICFilterSNI, set.UDP.FilterQUIC)
			}
		})

		t.Run("a nil set is skipped", func(t *testing.T) {
			cfg := NewConfig()
			cfg.Sets = []*SetConfig{nil}
			if err := migrateV51to52(&cfg, map[string]interface{}{}); err != nil {
				t.Fatalf("migration failed: %v", err)
			}
		})

		t.Run("applyMigrations carries a v51 file to the current version", func(t *testing.T) {
			cfg := NewConfig()
			set := NewSetConfig()
			set.UDP.FilterQUIC = "parse"
			cfg.Sets = []*SetConfig{&set}

			if err := cfg.applyMigrations(51, map[string]interface{}{}); err != nil {
				t.Fatalf("migration failed: %v", err)
			}
			if cfg.Version != CurrentConfigVersion {
				t.Errorf("version: want %d, got %d", CurrentConfigVersion, cfg.Version)
			}
			if set.UDP.FilterQUIC != QUICFilterSNI {
				t.Errorf("filter_quic: want %q, got %q", QUICFilterSNI, set.UDP.FilterQUIC)
			}
