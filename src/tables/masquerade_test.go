...
func masqueradeManifestBinaries(m Manifest) map[string]bool {
	out := map[string]bool{}
	for _, c := range m.Chains {
		if c.Table == "nat" && c.Name == masqChainName {
			out[c.IPT] = true
		}
	}
	return out
...
func TestMasqueradeTeardownCoversDisabledFamily(t *testing.T) {
	...
	if masqueradeManifestBinaries(applyManifest)[backendIP6Tables] {
		t.Errorf("apply manifest installs %s in %s while IPv6 is disabled", masqChainName, backendIP6Tables)
	}
	...
	bins := masqueradeManifestBinaries(teardownManifest)
	if !bins[backendIPTables] || !bins[backendIP6Tables] {
		t.Fatalf("teardown manifest %s chains = %v, want both families", masqChainName, bins)
	}
}
