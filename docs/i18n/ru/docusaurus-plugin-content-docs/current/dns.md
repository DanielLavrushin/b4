...
- Закреплённый адрес, до которого роутер не достучался по TCP на порт 443, в ответ не попадает. b4 подключается к каждому закреплённому адресу при старте, при сохранении сета и каждые `system.ip_health.retest_interval_sec` секунд (по умолчанию 300), а переставший отвечать адрес перепроверяется с тем же интервалом. Раунд, в котором не ответил ни один адрес, как при обрыве аплинка, ничего не доказывает и вердиктов не меняет. Когда недоступны все адреса семейства, закрепления всё равно отдаются в ответ, так что закреплённый хост, который просто не слушает порт 443, своё закрепление сохраняет; только при включённом обнаружении блокировки по IP такой запрос уходит на резолвер.
