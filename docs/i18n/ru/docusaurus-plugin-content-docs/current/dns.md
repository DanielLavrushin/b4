...
- Список закреплений - это весь адресный ответ для имени. Запросы A и AAAA получают закреплённые адреса соответствующего семейства; запрос A или AAAA для семейства, адресов которого в списке нет, а также запросы CNAME, SVCB, HTTPS и ANY получают пустой ответ, а не ответ резолвера, потому что такие ответы несли бы CNAME сайта, и системный кэш уходил бы по нему мимо закреплений, как только истекал ответ с закреплёнными адресами. Остальные типы записей, среди них MX, TXT и SRV, по-прежнему уходят на резолвер.
- Закреплённый адрес, до которого роутер не достучался по TCP на порт 443, в ответ не попадает. b4 подключается к каждому закреплённому адресу при старте, при сохранении сета и каждые `system.ip_health.retest_interval_sec` секунд (по умолчанию 300), а переставший отвечать адрес перепроверяется с тем же интервалом. Когда недоступны все адреса семейства, закрепления всё равно отдаются в ответ, так что закреплённый хост, который просто не слушает порт 443, своё закрепление сохраняет; только при включённом обнаружении блокировки по IP такой запрос уходит на резолвер.
