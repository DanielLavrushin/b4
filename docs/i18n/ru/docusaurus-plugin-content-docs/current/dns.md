...
:::note
В режиме TUN запросы на порт 53 перехватываются, а ответы видны только тогда, когда b4 держит весь маршрут по умолчанию, а запрос, адресованный собственному резолверу роутера, получает ответ раньше, чем его увидит b4. Перенаправления и закрепления работают в любом случае, потому что эти ответы b4 формирует сам. См. [Движок захвата](./settings/engine#dns-к-собственному-резолверу-роутера-не-перехватывается).
:::
