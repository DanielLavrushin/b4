...
sing-box, клиент вида `tun2socks` или Xray, запущенный руками, а не через [XrayUI](https://daniellavrushin.github.io/asuswrt-merlin-xrayui/ru/install).
...
маршрутизации. [XrayUI](https://daniellavrushin.github.io/asuswrt-merlin-xrayui/ru/install) ставит свои, b4 в режиме выходного интерфейса ставит свои. Какие
...
3. **Таблица по `ip rule`.** Ядро идёт по правилам от меньшего номера приоритета и
   ...
   `10000 + номер таблицы`. [XrayUI](https://daniellavrushin.github.io/asuswrt-merlin-xrayui/ru/install) ставит своё с приоритетом 51, и оно совпадает со всей
   локальной подсетью:
...
- Сплошное правило на стороне Xray придётся убрать. В [XrayUI](https://daniellavrushin.github.io/asuswrt-merlin-xrayui/ru/install) это отключение его прозрачной
...
исключение `/32`, которое [XrayUI](https://daniellavrushin.github.io/asuswrt-merlin-xrayui/ru/install) пишет в свою таблицу, и оно должно пережить любую из схем.
...
того, как его увидит сеть. Такие соединения несут пометку `routed-><iface>`: на странице
`Логи` она завершает их строки, а на странице `Трафик` их находит фильтр `flags:routed`.

Вкладки стратегии по-прежнему показывают свои настройки. Пока сет маршрутизируется в
туннель, они не применяются.

## Правило TPROXY от XrayUI и соединения, которые перенаправляет b4

При каждом запуске xray XrayUI добавляет в начало mangle PREROUTING такое правило:

```sh
iptables -t mangle -I PREROUTING 1 -p tcp -m socket --transparent -j DIVERT
```

`DIVERT` ставит пакету метку `0x10000` и принимает его. Правило совпадает с любым пакетом, адресованным локальному прозрачному сокету, а соединения, которые b4 перенаправляет на свои слушатели, заканчиваются именно на таких сокетах: сеты в режимах маршрутизации *Вышестоящий прокси SOCKS5* и *Telegram через WebSocket* и переключатель «Telegram через WebSocket». Принятый этим правилом пакет до правила b4 не доходит и, если никакое `ip rule` не направляет метку `0x10000` на сам роутер, уходит по маршруту по умолчанию. Соединение клиента тогда так и не завершает рукопожатие со слушателем b4.

На iptables b4 держит свои переходы маршрутизации выше любого правила mangle PREROUTING, которое проверяет локальные сокеты. Если такое правило оказывается выше них, следующая проверка файрвола возвращает переходы b4 выше него и пишет в лог предупреждение с именем правила; проверка идёт с **Интервалом мониторинга фаервола** из раздела Настройки, Основные, по умолчанию раз в 10 секунд. До этого карточка «Telegram через WebSocket» показывает **Не работает** и называет правило, а **Проверить снова** на карточке возвращает переходы сразу. Пока мониторинг файрвола выключен, то есть интервал равен `0` или включено **Пропустить настройку IPTables/NFTables**, проверок нет, и порядок восстанавливают только **Проверить снова** или сохранение настроек.

```sh
iptables -t mangle -L PREROUTING -n --line-numbers
```

Порядок верный, когда каждая строка `b4r_*_pre` стоит раньше `DIVERT`.
...
#    Трафик в веб-интерфейсе: если единственный источник - сам роутер,
#    остальное отсекает фильтр сетевых интерфейсов
```
