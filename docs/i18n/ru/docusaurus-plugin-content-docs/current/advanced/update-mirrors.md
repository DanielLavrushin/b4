...
В окне есть ссылки на обе страницы релиза, поскольку у машины с браузером доступа к GitHub
тоже может не быть. **Зеркало SourceForge** открывает папку этой версии по адресу
`sourceforge.net/projects/b4core/files/<tag>/`, где лежат те же архивы и те же файлы
`.sha256`, что и в релизе на GitHub.

Перед тем как что-либо заменить, сервис проверяет именно ту запись, которую установщик
поставит на место: ровно один файл `b4` в корне архива, достаточного размера, содержащий
исполняемый файл Linux, собранный под эту машину. Второй `b4` глубже в дереве архива
устанавливаться не будет и проверку не проходит. Архив
...
:::warning Должны быть заданы имя пользователя и пароль
Веб-сервер требует аутентификацию только после того, как заданы оба значения, и по умолчанию
слушает на всех интерфейсах. Загрузка файла заменяет бинарный файл, работающий от root,
поэтому этот путь не работает вовсе, пока веб-сервер принимает любой запрос, дошедший до
порта. Учётные данные задаются в разделе **Настройки** -> **Веб-сервер**.
:::
