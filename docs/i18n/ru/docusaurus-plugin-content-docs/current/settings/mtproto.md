...
| Порт relay | `0` отдаёт релей на порту веб-сервера, только по хосту релея. Любое другое значение открывает собственный слушатель, который отвечает на хост релея как релей, а на любое другое имя или IP - страницей-заглушкой, так что веб-интерфейс через него недоступен. Не может совпадать с портом веб-сервера или MTProto-прокси. | `0` |
| Файл сертификата, Файл закрытого ключа | PEM-пара для порта релея. Пусто: пара веб-сервера. Оба поля либо ни одного. | пусто |
