...
| **Разрешить активные проверки** | Позволяет ИИ загрузить домен с роутера и посмотреть, открывается ли он, а также запустить Дискавери для подбора рабочей стратегии обхода. По умолчанию выключено; это отдельное разрешение от изменения настроек: выпускать трафик - не то же самое, что записывать настройки. |
...
| --- | --- | --- |
| `b4_status` | Версия, движок захвата, бэкенд файрвола, сколько сетов есть и включено, состояние моста «Telegram через WebSocket», аптайм | «Работает ли b4 и какой движок захвата активен?» |
...
`b4_find_bypass_strategy` принимает необязательный `set` в том же виде. `action=start` с `set` запускает [Дискавери для этого сета](../discovery#запуск-для-сета): на переданных доменах или, если их нет, на адресах Дискавери сета, не больше пяти, с первой проверкой текущей стратегии сета и остановкой поиска на первой стратегии, подтверждённой на всех адресах. `action=status` затем сообщает вердикт сета, а `action=apply` с `set` записывает результат в этот сет - только стратегию, не трогая его домены, и только при вердикте `covered`. То же правило действует для `action=apply` без `set`, если запуск, на который он указывает по идентификатору или как последний, был запуском для сета. Сет, который `action=apply` создаёт для произвольных доменов, сохраняет адреса, на которых они найдены, как свои адреса Дискавери. Сет с включённой маршрутизацией отклоняется. Для запуска нужно **Разрешить активные проверки**, для применения - **Разрешить изменение настроек**.
...
- конфигурация изменилась после отменяемого изменения - восстановлением [мониторинга](../watchdog), в веб-интерфейсе или другим инструментом: возврат старой копии перезаписал бы и это более новое изменение;
