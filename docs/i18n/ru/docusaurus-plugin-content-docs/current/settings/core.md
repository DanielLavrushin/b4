...
:::info Вывод в консоль на OpenWrt
Init-скрипт для procd передаёт консольный вывод b4 в системный журнал, поэтому он виден в `logread`. Флаг `--console-level` или переменная окружения `B4_CONSOLE_LEVEL` ограничивают то, что попадает в консоль, независимо от уровня логирования; init-скрипт задаёт значение `error`, поэтому в `logread` попадают только ошибки и предупреждения, а веб-интерфейс получает полный поток. Оба учитываются на любой платформе; если ничего не задано, в stderr идёт всё на настроенном уровне.
:::
