...
**Режим захвата** определяет, как b4 получает пакеты из ядра: NFQUEUE по умолчанию или TUN. Значение читается при старте и требует перезапуска сервиса. Выбор TUN открывает группу настроек TUN и скрывает расположенные ниже элементы управления файрволом и интерфейсами, потому что в этом режиме они не применяются.

:::info
TUN существует для ядер без модулей NFQUEUE и обрабатывает меньше, чем NFQUEUE. Что ему требуется, между какими двумя режимами захвата он выбирает и какие возможности при нём недоступны, описано в разделе [Движок захвата](./engine).
:::
