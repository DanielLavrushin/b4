...
С движком NFQUEUE значение 0 полностью отключает мониторинг правил. Если внешняя программа или скрипт удалит правила b4 - они не будут восстановлены.
:::

В режиме TUN в группе «Фаервол» остаются только NAT Masquerade и интервал мониторинга. С этим интервалом движок TUN проверяет свою цепочку захвата `B4_TUN` и переходы в неё, а монитор фаервола - правила маскарада, MSS clamp и сетов маршрутизации. Каждый возвращает то, что удалил собственный фаервол роутера, например когда роутер перезапускает фаервол после изменения проброса портов. В этом режиме интервал не меньше 10 секунд, и значение 0 ни одну из проверок не отключает. `SIGUSR1` запускает обе проверки, не дожидаясь интервала. NAT Masquerade не трогает устройство TUN: адрес источника у захваченных пакетов движок TUN сам заменяет на адрес внешнего интерфейса и держит это правило выше любого правила маскарада без указанного выходного интерфейса.

:::info Восстановления правил в «Информации о системе»
Когда TUN захватывает трафик по портам (в строке «Захват» стоит `ports`), «Информация о системе» сравнивает число правил в цепочке захвата с числом установленных b4. После первого восстановления там же появляется, сколько раз восстанавливались правила захвата и, в любом режиме движка, сколько раз монитор фаервола восстанавливал свои правила, со временем последнего восстановления. Растущий счётчик указывает на другую службу на роутере, которая переписывает фаервол.
:::
...
При уровне **Ошибка** раздел **Логи** почти ничего не показывает: он читает поток логов, который на этом уровне практически пуст. Раздел **Трафик** получает данные отдельным потоком, и уровень логирования на него не влияет.
:::
...
Прокси настраивается на вкладке **Telegram**, вместе с WebSocket-мостом и WEB-прокси. Список её полей - в разделе [Настройки, Telegram](./mtproto.md), а руководства по трём режимам работы с Telegram - в разделе [Telegram](../telegram/index.md).
