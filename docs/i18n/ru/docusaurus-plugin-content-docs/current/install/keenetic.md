...
После перезаписи в логе остаются строки `Tables rules missing, restoring...` и `Tables rules restored successfully`. Ту же перепроверку можно запросить вручную:
