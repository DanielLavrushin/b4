...
:::info Метки TLS
Рядом с доменами могут отображаться метки **1.2** или **1.3** - это версия протокола TLS, которую использует соединение. Такие же метки встречаются на страницах Трафик и Дискавери. Версия TLS важна, потому что провайдеры могут блокировать TLS 1.2 и TLS 1.3 разными методами - для них могут потребоваться разные стратегии обхода.
:::
