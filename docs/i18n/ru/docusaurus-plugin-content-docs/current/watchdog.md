...
| Правило | Результат |
| --- | --- |
| Проверка TLS-сертификата включена | Ошибка сертификата делает адрес **Непригодным**. |
...
После каждой попытки восстановления, удачной или нет, сет ждёт **Перерыв между discovery**, прежде чем снова проверяться. После трёх неудачных восстановлений подряд мониторинг прекращает поиск для сета (**Поиск прекращён**): проверки продолжаются с обычным интервалом, но восстановление не ставится в очередь, пока не произойдёт одно из:
...
| **Работает** | При последней проверке открылись все адреса, которые обрабатывает сет. |
