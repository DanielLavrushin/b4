...
:::info
Дискавери гоняет свои пробы через вторую очередь netfilter, поэтому на движке захвата TUN он отклоняется. См. [Движок захвата](./settings/engine#дискавери-недоступен).
:::
...
| Статус | Значение |
| --- | --- |
| **Успешно** | Найдена рабочая конфигурация. Показывает скорость и сколько подтверждающих попыток она прошла |
| **Обход не нужен** | Домен открывается без обхода DPI, поэтому стратегия для него не предлагается |
