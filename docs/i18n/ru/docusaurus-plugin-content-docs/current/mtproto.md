...
Дополнительный домен, который проксирует WebSocket-трафик к Telegram - для собственного релея вместо общего пула. B4 сам подставляет `kws1.`, `kws2.` и так далее по номеру дата-центра, поэтому одного домена хватает на все, и имя должно резолвиться для каждого используемого дата-центра. Пробуется после родного узла Telegram и перед общим пулом CF-доменов; от резерва через CF-прокси не зависит - каждый работает без другого.

### IP edge-сервера Telegram

Все подключения с родным именем `kws*.web.telegram.org` идут на один адрес, `149.154.167.220`, а дата-центр выбирается по имени, а не по адресу. Эта настройка заменяет этот адрес - для сети, в которой адрес по умолчанию не отвечает. Принимает хост или IP без порта, пустое значение оставляет адрес по умолчанию. Свой WebSocket-домен резолвит собственное имя, и на него это не влияет.

:::warning
Изменение любой из этих настроек перезапускает MTProto-прокси и обрывает переносимые им сессии. Telegram переподключается сам.
:::
