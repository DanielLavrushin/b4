...
B4 может сохранять работу Telegram в сети с цензурой двумя независимыми способами:
...
Settings → **General** → **MTProto Proxy** → **Telegram upstream**. Определяет, как B4 достигает дата-центров Telegram. Используется и прокси-сервером, и режимом WebSocket-моста, поэтому действует даже когда прокси-сервер выключен.
...
Если не загружаются медиа, реакции или стикеры, укажите **домен Cloudflare Worker**. Это бесплатный персональный WebSocket-релей на `workers.dev`, который достаёт все дата-центры и обходит лимиты общих доменов. Несколько воркеров указывайте через запятую. Инструкция - по ссылке рядом с полем в интерфейсе.
...
1. Создайте или откройте сет и используйте его с geoip-целью **`telegram`** (чтобы сет совпадал с диапазонами IP Telegram).
2. На вкладке **Routing** сета включите маршрутизацию и выберите **Режим маршрутизации** → **Telegram через WebSocket (встроенный)**.
3. Выберите **source-интерфейсы** (интерфейсы локальной сети, чьи устройства нужно переправлять).
4. Сохраните.
...
В **Settings** → **General** → **MTProto Proxy** укажите в поле **DC Relay** адрес VPS с базовым портом (например `my-vps.com:7007`). Поле появляется, когда режим транспорта - Прямой TCP или Авто.
