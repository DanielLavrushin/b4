...
| **Без эффекта** | Опция разобрана, но и в исходном инструменте ничего не делает. Например, `--md5sig` в профиле byedpi, который не отправляет фейковых пакетов |
...
- **Автоматический TTL** (`--dpi-desync-autottl`, замер числа хопов в byedpi). b4 использует фиксированный TTL для фейковых пакетов. Задайте его вручную или найдите через [Дискавери](../discovery).
