...
:::caution IPv6
Всё написанное ниже описывает то, что происходит в пределах семейств адресов, которые обрабатывает b4. Обработка IPv6 по умолчанию выключена, и тогда сет блокировки не пишет IPv6-правил вообще, поэтому заблокированное назначение, отвечающее ещё и по IPv6, остаётся доступным по IPv6. См. [IPv6](#ipv6) ниже.
:::
...
Правила блокировки пишутся отдельно для каждого семейства адресов, и b4 пишет их только для тех семейств, которые обрабатывает. **Поддержка IPv6** в [Настройки → Основные](../settings/core#протоколы) по умолчанию выключена, и при выключенной поддержке сет блокировки не создаёт IPv6-правил вообще.

Чего это стоит по слоям:

- **DNS-синкхол.** По-прежнему делает основную работу. NXDOMAIN скрывает имя от клиента независимо от того, какую запись он спрашивал - и A, и AAAA, - поэтому устройство, резолвящее через роутер, не получит адреса ни одного семейства. Исключение - запрос, который приходит на роутер по самому IPv6: пока поддержка IPv6 выключена, такой запрос пересылается без изменений, а не отправляется в синкхол, и в логе он отмечается как `dns-ipv6-disabled`. Именно так ведут себя устройства, которым адрес резолвера пришёл в router advertisement по IPv6.
- **Блокировка соединения по имени.** Не применяется. b4 не читает IPv6-пакеты, поэтому SNI в IPv6-рукопожатии TLS или QUIC никогда не проверяется.
- **Блокировка по адресу.** Только IPv4. IP, диапазоны CIDR и категории GeoIP применяются в файрволе для IPv4, а IPv6-элементы сета никуда не устанавливаются.

На практике это значит, что устройство, у которого уже есть IPv6-адрес заблокированного хоста - из шифрованного DNS, которого роутер не видит, или из собственного кеша, - откроет этот хост по IPv6 беспрепятственно.

Чтобы блокировка покрывала оба семейства, включите **Поддержку IPv6** в [Настройки → Основные](../settings/core#протоколы) и перезапустите сервис. Блокировать IPv6 на роутере таким образом - это не то же самое, что выключить IPv6 для сети: последнее делается настройками роутера вне b4.

## Охват

Режим блокировки действует на всю сеть в пределах тех семейств адресов, которые обрабатывает b4: он останавливает совпавшие назначения для каждого устройства в сети и для самого роутера. Ему не нужны выходной интерфейс, таблица маршрутизации или прокси, поэтому он работает на любой конфигурации, которую поддерживают остальные режимы маршрутизации.
