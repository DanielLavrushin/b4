...
- **Поддержка IPv6 включена.** IPv6-трафик сета по-прежнему уводится на выходной интерфейс, работает через masquerade и уходит с собственным IPv6-адресом интерфейса. Если этот адрес важен для вышестоящего устройства, задайте сету и IPv6-адрес egress.
