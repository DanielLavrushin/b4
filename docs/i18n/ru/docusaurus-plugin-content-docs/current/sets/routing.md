...
| Telegram через WebSocket (встроенный) | Перехватывает совпавший трафик Telegram и ретранслирует его через WebSocket-узел Telegram. См. [Telegram через WebSocket](../telegram/websocket-bridge.md). |
