...
только соединения к адресам, которые сет сопоставляет, - обычный трафик роутера его бюджет израсходовать не может, - и
ставится на каждый сет, который маршрутизирует трафик самого роутера: обычный интерфейс соединение заново не откроет, но
на момент построения правил интерфейса может ещё не быть, поэтому b4 не ждёт. На iptables нужен модуль `xt_hashlimit`: b4
загружает его сам и предупреждает, если в ядре его нет.
