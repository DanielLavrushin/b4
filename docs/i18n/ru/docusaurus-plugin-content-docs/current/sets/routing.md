...
:::note
Недоступно в режиме движка TUN с захватом всего маршрута по умолчанию. Там b4 переотправляет пакеты по пути, который обходит это правило, и настройка не действует. См. [Движок захвата](../settings/engine#egress-ip-сета-не-сохраняется-для-захваченного-трафика).
:::
