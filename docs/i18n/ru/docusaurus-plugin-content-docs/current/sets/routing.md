...
- Адрес уже должен быть настроен на выходном интерфейсе, например `ip addr add 192.168.1.51/32 dev br0`. b4 его не добавляет и не сохраняет между перезагрузками. Если адреса нет, b4 пишет предупреждение и возвращается к masquerade, вместо того чтобы отправлять трафик, на который никто не ответит.
...
Семейство адресов должно совпадать. IPv4-адрес подменяет только IPv4; IPv6-трафик сета продолжает работать через masquerade и уходит с собственным IPv6-адресом интерфейса.
