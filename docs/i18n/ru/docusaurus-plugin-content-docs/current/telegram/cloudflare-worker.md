...
Он стоит последним среди маршрутов WebSocket, после собственного узла Telegram и после общего пула CF, и вступает в гонку, когда они полторы секунды не могут подключиться, если только он не понижен после зависания.
...
:::info Сети, где замедляют workers.dev
В некоторых сетях TLS-рукопожатие с именем `workers.dev` завершается, а поток затем останавливается после первых нескольких килобайт. b4 засчитывает такое зависание Worker и после двух зависаний за пять минут откладывает его на десять минут. Собственные соединения b4 с Worker по умолчанию идут без обработки DPI; с включённым переключателем **Обрабатывать соединения с Worker сетами** к ним применяет свою стратегию сет, цели которого покрывают Worker. Подробнее в разделе [Обработка DPI](./websocket-bridge.md#обработка-dpi).
:::
