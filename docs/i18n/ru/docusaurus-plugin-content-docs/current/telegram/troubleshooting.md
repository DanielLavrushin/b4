...
В WebSocket-мосте дата-центр часто определяется по адресу соединения, а не со слов клиента, как описано в разделе [В какой дата-центр идёт сессия](./websocket-bridge.md#в-какой-дата-центр-идёт-сессия). Тогда `-444` означает, что адрес отнесён не к тому дата-центру, и маршрут не понижается: следующие сессии на этот адрес идут в другой дата-центр той же площадки, а если его нет, мимо маршрутов к дата-центрам.
