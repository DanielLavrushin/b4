...
Предупреждение приводит ошибку скачивания и называет источник списка, который используется сейчас. Не ответили ни `core.telegram.org`, ни зеркало b4. Используемый список остаётся, из какого бы источника он ни был взят, а встроенный список входит в него всегда. Следующая попытка идёт через 30 секунд, а после каждой новой неудачи пауза удваивается, но не больше часа; после успеха список обновляется раз в сутки. **Обновить адреса** повторяет попытку сразу.
