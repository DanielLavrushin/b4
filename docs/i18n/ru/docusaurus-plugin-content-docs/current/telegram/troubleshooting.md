...
Маршрут принял две записи или больше и не ответил ни на одну: первая ждёт восемь секунд, последняя - три. Одиночная запись, на которую ответ не нужен, например подтверждение в простаивающей сессии, не считается. Так ведёт себя Cloudflare Worker, и b4 понижает его в приоритете на десять минут.
