...
Режиму нужны модули ядра `tproxy` и `socket`; без них b4 пишет, что прозрачное перенаправление недоступно, и не устанавливает правил. **Настройки -> Диагностика** показывает, доступен ли TPROXY, и называет недостающие модули вместе с пакетами, которые их содержат; тот же список есть в разделе [Маршрутизация, требования](../sets/routing.md#требования).
