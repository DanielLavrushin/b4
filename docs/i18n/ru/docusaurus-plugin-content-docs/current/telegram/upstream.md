...
Настройки, Telegram, **Транспорт к Telegram (общий)**. Эти настройки определяют, как сам b4 доходит до дата-центров Telegram. Их используют и [прокси-сервер](./mtproto-proxy.md), и [WebSocket-мост](./websocket-bridge.md), поэтому они действуют даже при выключенном прокси-сервере.
