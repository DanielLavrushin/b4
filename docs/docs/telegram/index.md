...
- **[Telegram over WebSocket](./websocket-bridge.md)** - a switch on Settings, Telegram. b4 diverts Telegram's TCP connections from every device behind it, and from the router itself, into a bridge that relays them, so nothing is configured on the device. A per-set routing mode feeds the same bridge for chosen devices or interfaces only.
...
| --- | --- | --- | --- |
| Configured in | Settings, Telegram, **MTProto Proxy** | Settings, Telegram, **Telegram over WebSocket**, or a set's routing mode | Settings, Telegram, **Telegram Desktop WEB proxy** |
| On the client | A proxy entry with a secret | Nothing | A proxy entry from a link |
| Clients covered | Any Telegram client | Every device behind b4, and the router itself | Telegram Desktop 7.1.1 and later |
...
| Needs TPROXY in the kernel | No | Yes | No |
...
:::info
The per-setting reference for the Telegram tab, including the fields none of these pages walk through, is [Settings, Telegram](../settings/mtproto.md).
:::
