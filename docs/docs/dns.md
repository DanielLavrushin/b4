...
- A pinned address the router cannot reach on TCP port 443 is left out of the answer. b4 connects to every pin when it starts, when a set is saved and every `system.ip_health.retest_interval_sec` seconds (300 by default), and an address that stops answering is tried again at the same interval. A round in which no pin answers at all, as during an uplink outage, proves nothing and changes no verdict. When every pin of a family is unreachable the pins are answered anyway, so a pinned host that simply does not listen on port 443 keeps its pin; only with IP block detection on does such a query go to the resolver instead.
