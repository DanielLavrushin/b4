...
- The pin list is the whole address answer for the name. A and AAAA queries are answered with the pinned addresses of the matching family; an A or AAAA query for a family the pin list has no address in, and the CNAME, SVCB, HTTPS and ANY queries, get an empty answer rather than the resolver's, because those answers would carry the site's CNAME and the system cache would follow it past the pins once the pin answer expired. Other record types, MX, TXT or SRV among them, still go to the resolver.
- A pinned address the router cannot reach on TCP port 443 is left out of the answer. b4 connects to every pin when it starts, when a set is saved and every `system.ip_health.retest_interval_sec` seconds (300 by default), and an address that stops answering is tried again at the same interval. When every pin of a family is unreachable the pins are answered anyway, so a pinned host that simply does not listen on port 443 keeps its pin; only with IP block detection on does such a query go to the resolver instead.
