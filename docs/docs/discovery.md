...
| **Intercepted by the LAN gateway** | The TCP handshake to the site's addresses is answered by the router in front of this host, not by the site: a SYN sent with a hop limit of one, which nothing beyond the first hop can answer, gets a reply. A transparent proxy on that router carries the connection, so no packet strategy from this host reaches the ISP, and the run stops after the DNS phase when every address is served that way. b4 has to run on that router, or this host has to be excluded from its redirect. When another region's DNS answer gives an address that routes normally, the search goes on with that address instead. |
