...
- ADDED: **Written facts about a setting that the AI reads before explaining or changing it** - b4 field names are often misleading and a zero usually means "use the fixed value" rather than "off", so a model answering from the name alone invented units and defaults. `b4_get_topic` returns the description for one setting, searches them, or lists every documented key, and the same text is what the `b4://topics/<key>` resources hold.
- ADDED: **Searching the geosite and geoip databases from the AI** - nothing could say which category covers a domain or an address, so a category name was guessed, and b4 accepts an unknown one and silently matches nothing. `b4_geo_lookup` lists categories, previews what one holds and reports which of them cover a given domain or IP.
- ADDED: **Changing what a set targets and which sets exist, from the AI** - settings inside a set could be written, but its domain, address and geo lists could only be replaced wholesale, which skipped entry validation and the handover that moves a domain off whichever set already held it, and a set itself could not be created, duplicated, reordered or removed at all. `b4_edit_set_targets` adds and removes entries one at a time; `b4_manage_set` covers the lifecycle, and refuses to delete or reset a set holding credentials or DNS pins.
- ADDED: **Allow active probes, a permission that lets the AI make the router fetch a site** - no tool answered whether a domain actually loads: coverage said a set targets it and recent connections said traffic arrived, neither of which is the same question. `b4_test_domain_now` fetches a domain once through b4 and once with b4 bypassed, `b4_find_bypass_strategy` runs Discovery and turns the winner into a set, and private and local addresses are refused. Off by default and separate from configuration changes, because emitting traffic is a different act from writing a setting. `Settings > Integrations`.
- FIXED: **A finished Discovery run offered over MCP could not be applied a minute later** - the AI was handed a run id, told the search had found a working strategy, and then refused when it asked to install it, with the explanation that the result was not saved. It is saved: b4 writes every finished run to its Discovery history, and the web interface has always applied from that list. Only the MCP tool looked at the in-memory copy, which is dropped 30 seconds after a run ends. `action=status` and `action=apply` read the history too, addressed by the run id or by the domain alone, and history keeps the set the run built so it survives a restart.
- FIXED: **The Discovery history file grew by roughly half a megabyte per domain tested** - every strategy a run tried was saved with a full copy of the set b4 had built to test it, so one domain that went through 230 presets wrote 616 KB, and the hundred entries the file keeps could reach 60 MB on a router whose flash has far less than that. Only the winning strategy's set is worth keeping, and that is what is written; the same entry costs 25 KB.
- ADDED: **The transcript of a failed update, readable from the AI** - when an update started from the web interface left b4 broken, what the installer printed went to `update.log` and stayed there: no API route and no page in the interface ever showed it, so the one file explaining the failure needed an SSH session to read. `b4_logs_tail` takes `file=update` for it, and reads the rolled-over half of either log when the current one is short.
- FIXED: **The AI was told b4's error log holds warnings and ordinary activity** - only errors are ever written to `errors.log`, at any log level, so a model that found it empty could report that nothing had gone wrong when it had simply looked in a file that records one kind of line. The tool says which log it read and what that log does not contain, and the documented advice to raise the log level and read it back is corrected: verbosity reaches the console and the interface's live view, never the file.
- ADDED: **A record in the log of every MCP tool call and every request the MCP endpoint turns away** - b4's MCP server can rewrite strategy sets and make the router fetch a site, and nothing it did left a trace: a client guessing at the access token was refused silently, so an operator had no way to tell an AI assistant apart from someone probing the port. Each call writes the tool, the caller and the duration, a refused call is a warning, and repeated refusals collapse into one line every 30 seconds with a count so a guessing client cannot flood the log. The token is never written.
- CHANGED: **Every MCP tool was advertised whatever the permissions were, and refused at the moment it was called** - a model spent context on tool descriptions it could not use and read a refusal as the feature being absent. The served list is built from the two switches and rebuilt whenever they change, and the server tells the model which capability is off and what to turn on.
- FIXED: **The watchdog judged every watched domain on a fetch that skipped b4's own bypass** - its checks carried the mark b4 puts on traffic it has already handled, so the firewall let them past the packet engine untouched and the verdict described the unprotected path rather than the set meant to be protecting that domain. The same mark made a self-healed strategy impossible to verify, so healing escalated against a check that could never pass.
