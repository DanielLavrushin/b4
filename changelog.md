...
- ADDED: **A UDP action that leaves matched packets alone** - all three UDP actions interfered with the traffic, so a set matching QUIC only to learn a site's addresses, or to carry that traffic somewhere else, had to fake and fragment packets it had no reason to touch.
- CHANGED: **Two of the three QUIC matching values did the same thing under names that promised otherwise** - one read as turning QUIC matching off and one as merely parsing, while both matched the set by the server name in the ClientHello, the only way a set with domain targets matches UDP at all. They are one value, By SNI, and stored sets are converted on the first start.
- ADDED: **A Block QUIC switch on a set's UDP tab** - pushing a browser off QUIC and back onto TCP is the one countermeasure that works against a censor blackholing UDP port 443, and it took two settings in two places on the tab, with nothing saying they belonged together. `Sets > UDP`.
- FIXED: **An IPv6 packet carrying extension headers could crash b4** - each header's length was added to the offset without checking it against the packet, so a header claiming more bytes than were there sent the read past the end of the buffer.
- ADDED: **Force IPv4 for matched domains, which drops the IPv6 records from answers for names a set matched** - b4 processes IPv4 only until IPv6 support is turned on, so a dual-stack site a set targets was reached over IPv6, where b4 has no rules at all, and the set was bypassed with nothing in the log to say so. `Settings > Core > DNS`.
- FIXED: **A DNS answer that b4 rewrote never cleared the failure count for that name** - the answer was judged before the rewrite rather than after, so a good answer that had addresses replaced counted against the name, and a site that recovered was moved to its backup set on the next single failure instead of after the configured run.
- ADDED: **An egress IP for a set, which replaces the source address of its traffic instead of leaving the output interface's own** - a set could only be steered by naming an interface or an upstream SOCKS5 proxy, so where the tunnels are terminated by a router above b4, the tunnel had to be brought onto b4's own host once per destination, or every packet relayed through a proxy. b4 puts the address on the interface itself, ARP-probes it first and takes it back when the set stops using it.
- FIXED: **A set bound to a manually added source device stopped taking its route** - such a device has no MAC address on the network, so the firewall rule was built from the placeholder b4 derives from its IP and matched no packet, while the connection log still named the set. The same placeholder disarmed block sets, the QUIC rejection that pushes a client back to TCP, and "all devices except selected", which kept routing the one device it was told to leave alone.
- CHANGED: **The global device filter skipped devices added by hand** - ticking only those under Settings, Device Filtering left the filter inert and routing kept applying to every device on the network. They count towards the routing side of the filter, matched by the IP address entered for them; the DPI bypass side selects by MAC alone, so that half stays inert for them.
- FIXED: **An MSS clamp on a device added by hand clamped nothing and pulled every other forwarded connection down with it** - the rule was built from the placeholder MAC and matched no packet, and because iptables cannot narrow the reply direction by MAC at all, b4 covered it with one catch-all that clamped every returning connection through the router to the smallest configured size.
- FIXED: **On a router with working IPv6, Discovery stopped after about twenty tests and reported that no bypass was needed** - its test connections were free to use either version of the protocol while b4 acts on IPv4 alone, so the opening test, the one that decides whether anything is blocked at all, succeeded over a route nothing was interfering with.
- FIXED: **A routing set limited to source devices also diverted traffic the router itself sent** - the rules on the router's own outgoing traffic carried no device filter, and paired with "Match any IP address" that pulled the router's whole outbound path into the tunnel and took LAN reachability with it.
- FIXED: **A routing set with no domain or IP target was built in full and quietly matched nothing** - a mark, a routing table, two kernel address sets, three chains and the firewall jumps were installed for a set that cannot match a packet, because every routing rule matches a destination address, and nothing above trace level said so.
- FIXED: **A set matched only by TCP or UDP port ignored the source devices it was limited to** - the port match ran before any source check, and the set also claimed its traffic was being diverted, which switched off every DPI countermeasure while the firewall steered nothing.
...
- CHANGED: **The Cloudflare Worker is asked for a data center address without a port** - data center 203 does not answer on 5222, one of the three ports Telegram serves the same endpoint on, so passing the port on killed media connections. The script in the [Telegram documentation](https://daniellavrushin.github.io/b4/docs/mtproto) is again the tg-ws-proxy script unchanged, and a Worker deployed from either page needs nothing done to it.
...
- ADDED: **A documentation page for DNS** - what b4 does with name lookups was written down only in pieces, beside each setting it belonged to. [DNS](https://daniellavrushin.github.io/b4/docs/dns).
