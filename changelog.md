...
- FIXED: **Stopping b4 left the router unable to finish a large download** - b4 relaxes the kernel's TCP window tracking while it runs, because the packets it crafts sit outside the window on purpose, and it put the router's own setting back on the way out. On a router that tracked windows strictly and whose firewall drops what that marks invalid, that setting is the one that lets a page load its first fifty kilobytes and then stop, with no reset and no error, so a browser sat on half-loaded pages and the fault read as b4's. Where putting it back would do that, b4 leaves the relaxed value and names the setting to fix on the router itself.
- FIXED: **On a Broadcom router every connection a set routed hung after the handshake** - the route a connection takes is decided on its first packet and carried on the connection from then on, and that decision is kept in the connection mark, which on these boxes the firmware hands out itself, one value per forwarded flow, overwriting b4's before the second packet arrives. The first packet left by the set's interface, every packet after it by the ordinary uplink, and the far end dropped them as belonging to no connection it had. b4 checks whether the mark it writes is still there and, where it is not, marks every packet the set matches rather than only the first, and remembers that about the router so it is not worked out again from scratch on every restart.
- FIXED: **A set stopped routing whenever the router rebuilt its own firewall, and nothing put it back** - the check that decides whether b4's rules are still installed looked for b4's own chains, which survive a firmware firewall pass untouched, and never for the jump in PREROUTING, OUTPUT or POSTROUTING that carries a packet into them, so after a WAN reconnect, a DHCP event or `service restart_firewall` the set read as healthy while no packet reached it. The jump is checked too, and the waiting b4 does for a firewall another program is rewriting is capped for the whole pass rather than per command, which had it holding its own lock for minutes.
- FIXED: **A busy capture engine took the network down with it instead of stepping aside** - the firewall rules that hand traffic to b4 carry a bypass, but that only covers a b4 that is not running: a b4 that is running and behind has its queue filled, and the kernel drops every packet past the four thousandth with nothing to say why, which on a router is the whole network's forwarded traffic. The queue is opened asking the kernel to let a packet through rather than drop it when the backlog is full, and a kernel too old for that is named in the log.
- FIXED: **On a router whose iptables has no lock flag, part of b4's firewall rules were silently never installed** - the commands run unserialised there, the firmware's own firewall scripts rewrite the same tables on WAN checks and DHCP events, and a command that lost that race failed with nothing but a warning, leaving b4 on half a rule set, so one start was fine and the next took the network off the air until a reboot rebuilt the firewall. b4 waits for the lock itself and retries, and reports the rule it had to give up on, but never for a command that removes a rule by its position: that failure means the table changed under the read the number came from, and repeating it deletes whichever rule has moved into that slot, which on such a router is the firmware's own NAT or INPUT rule.
...
- FIXED: **A set routed to a TUN interface a proxy reads took the whole router's memory with it, usually within the hour** - the set's rules marked the connections the router opens itself, and a proxy with a TUN inbound does not forward a packet, it opens its own connection to the same destination from the same box. That destination is in the set, so b4 marked the new connection and routed it straight back into the TUN the proxy was reading. Every turn carried a new source port, so nothing in the kernel saw it as a repeat, and every turn cost the proxy another session and another socket. b4 now recognises a TUN or TAP device the kernel opened for a userspace program and leaves the router's own traffic on the normal route there; a new **Router's own traffic** setting on the routing tab overrides it either way, and choosing to route it into such an interface anyway is capped at 200 new router-originated connections per second so a loop cannot run away. An interface that is not present yet counts the same way, because b4 cannot tell what it is: starting before the program that creates it otherwise handed the router's own connections to a set whose table holds nothing but the kill-switch route, and each one failed the instant it was opened and was retried as fast as the processor allowed.
- FIXED: **b4 could claim a routing table another program was already using, and for a set with an upstream proxy that took the router's name resolution down with it** - the number came from the output interface's name for a routed set and was fixed at 252 for a proxy set, with nothing checking `/etc/iproute2/rt_tables` or the rules already pointing at that number. Where 252 was a hand-made `vpn` table, enabling the proxy set wrote a local-delivery route into it, so every address the user sent through that table, upstream DNS servers included, was answered by the router itself and every lookup on the box timed out. b4 reads the named tables and the live rules before it picks, and logs the number it took.
- FIXED: **A set routed into a proxy's interface could take the router down within seconds of the service starting** - the routing rules pick a packet by its destination alone, with nothing asking where it came in from, so a packet the proxy handed back for the same destination was picked again and sent straight back to that proxy. One packet from one client cycled thirty-one times before its hop count ran out, and with a whole network behind such a set the router stops forwarding almost at once. On most systems the kernel's reverse-path check was silently dropping those packets, which hid the loop and cost the set every reply it should have delivered. Traffic arriving on the interface a set routes to is left alone.
- FIXED: **The first connection to each address a set routes went out one path and finished on another, and hung** - the kernel decides where to send a packet by looking the destination up in the set's address list, and for a domain target that list only learns the address when b4 sees the answer to the lookup, which is a moment after the connection has already started. The handshake left by the ordinary uplink, the address arrived, and the rest of the connection was sent down the set's route, where nothing knew about it. The route a connection takes is decided on its first packet and carried on the connection from then on, so a flow goes one way or the other and never both.
- FIXED: **A set routed out an interface answered nothing on a router with strict reverse-path filtering** - the kernel checks that a reply arrives on the interface it would have used to reach that address, and for a set the reply comes back through the set's interface while the address itself routes over the ordinary uplink, so every answer was dropped before a single rule saw it: no error, no reset, just a connection that hangs. The capture engine already relaxed this for its own uplink and the routing side did not. b4 relaxes it on the set's interface for as long as a set uses it and puts the previous value back afterwards.
- FIXED: **On a router that already routes by policy, the TUN engine captured nothing from the network and hijacked the proxy's own connection instead** - the capture rule sat at routing-rule priority 90, below the rules a transparent proxy such as XRAYUI, a multi-WAN manager or a VPN script installs, so every packet from the network was claimed by those before b4 could see it and the bypass did nothing. What b4 did capture was traffic the router itself opened, which on such a setup is the proxy's own upstream connection, and it re-injected that by its own uplink with the router's policy routing discarded. Capture is consulted ahead of other policy routing, everything b4 re-injects follows the routes the router already had, and a network that still cannot reach the tunnel is named in the log and in Settings, System Info.
- FIXED: **On the TUN engine, a connection between two of the router's own networks was sent out through the WAN** - the capture rules match a port without regard for where the packet is going, and the table they steer into holds a default route and nothing else, so a request from the LAN to a device on another bridge, or to a container, was carried into the tunnel, given the uplink's address and sent to the internet. The directly connected networks are left out of capture and refreshed whenever an address changes.
- FIXED: **On the TUN engine, b4 never saw a DNS answer** - only queries were carried into the tunnel, so the half of the DNS handling that reads replies did nothing: no addresses learned into a routing set, no AAAA stripping, no dead-address healing and no escalation after a failed lookup. Replies from port 53 are carried in as well and sent back towards the client instead of out the uplink.
- FIXED: **On a router without IPv6, the Telegram bridge gave up on an edge whose name also carries an IPv6 address** - b4 asked the resolver for every address and dialled whichever came back first, and a connection to an IPv6 address on a box with no IPv6 route is refused by the kernel outright, so an edge that answers perfectly well over IPv4 was recorded as a failure. b4 asks for an IPv6 address only when the router has both a global address and a default route for it.
- FIXED: **A set with routing turned on but left unfinished installed nothing and said nothing** - a set on the interface mode with no output interface chosen, or on proxy mode with no upstream port, was dropped from the routing pass without a single line in the log, so its rules were absent while the interface still showed the set as routed. Such a set is named in the log together with the field it is missing.
- ADDED: **A kill switch for a set's output interface** - when the interface went away the kernel removed the route b4 had put in the set's table, the mark rule found the table empty, and the lookup fell through to the main table, so the set left through the ordinary uplink with the router's real address while its rules still read as correct. Turned on, b4 keeps a blackhole route in the set's table at a worse metric than the real one, so the set stops instead of leaking, and starts again by itself when the interface returns. It arms only once the interface has existed: a blackhole ends a route lookup rather than falling through to the main table, so arming it for an interface that had never appeared dropped every address the set matched inside the kernel, with no ICMP and no reset, and on a set built from geosite categories that was most of what the network browses. `Sets > Routing`.
