...
- ADDED: **Discovery and the DPI Detector recognise a LAN gateway that terminates TCP itself** - a SYN sent with a hop limit of one can only be answered by the next hop, so an address that answers it is served by a transparent proxy on the router, not by the site. Discovery leaves such addresses out of the search, goes on with addresses from the ECS scan that route normally, and stops with a clear verdict when nothing is left; the Detector reports the site as `GATEWAY` with a note that b4 on this host cannot change it.
...
- FIXED: **A hub address that failed the sync was reported as `did not answer the health check` whatever the reason** - the cause, a failed lookup, a refused connection, a timeout or the status code, was dropped on the way to the status line.
