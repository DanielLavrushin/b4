...
- ADDED: **Cloudflare Worker support for the MTProto WebSocket upstream (free, per-user)** - a new "Cloudflare Worker domain" field in Settings -> MTProto. A Worker is a free personal WebSocket relay on workers.dev (no domain purchase needed, unlike CF-proxy) that reaches every Telegram data center and is not shared with other users, so it avoids the rate limits that make media, reactions, and stickers fail to load. When set, b4 tries it right after Telegram's own edge and before the shared fallback domains. Works for both the proxy server and the WebSocket routing (bridge) mode. The field links to the setup guide; multiple workers can be comma-separated.
- FIXED: **Some media failed to load over the Telegram WebSocket routing (bridge mode)** - b4 keeps a pool of ready WebSocket connections to Telegram, but Telegram closes idle ones server-side. A connection could die in the brief moment between b4's health check and first use, so a media request was sent and nothing came back (a short dead session). Media downloads open many parallel connections at once, so a stale pool hit a lot of them while chats kept working. b4 now re-checks each pooled connection at the last instant before use and dials a fresh one if it died, and keeps pooled connections younger so fewer go stale.
- FIXED: **Telegram data centers without a WebSocket edge (DC1/DC3/DC5) could stall under load** - those data centers are reached through a shared pool of Cloudflare-proxied domains, which get rate-limited (429/503) in bursts. b4 retried all of them on every connection, so once they were throttled everything stalled. b4 now puts a rate-limited domain aside for a minute before trying it again, and only falls back to trying throttled domains when no others are left.
