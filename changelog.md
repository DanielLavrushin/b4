...
- FIXED: **Turning domain-only matching off left the set routing nothing until the service was restarted** - the flag decides whether b4 puts a domain's resolved addresses into the set's address list, but it was not part of what b4 compares when a set is saved, so the set kept its cached state, its rules were never rebuilt, and the pass that resolves its domains never ran. The list stayed empty and the set carried no traffic until a client's own DNS cache happened to expire, which reads as the setting having no effect at all.
- FIXED: **Stopping b4 left the router unable to finish a large download** - b4 relaxes the kernel's TCP window tracking while it runs, because the packets it crafts sit outside the window on purpose, and it put the router's own setting back on the way out.
- FIXED: **On a Broadcom router every connection a set routed hung after the handshake** - the route a connection takes is decided on its first packet and carried on the connection from then on, and that decision is kept in the connection mark, which on these boxes the firmware hands out itself, one value per forwarded flow, overwriting b4's before the second packet arrives. The first packet left by the set's interface, every packet after it by the ordinary uplink, and the far end dropped them as belonging to no connection it had.
- FIXED: **A set stopped routing whenever the router rebuilt its own firewall, and nothing put it back** - the check that decides whether b4's rules are still installed looked for b4's own chains, which survive a firmware firewall pass untouched, and never for the jump in PREROUTING, OUTPUT or POSTROUTING that carries a packet into them, so after a WAN reconnect, a DHCP event or `service restart_firewall` the set read as healthy while no packet reached it.
