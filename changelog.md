...
- ADDED: **A kill switch for a set's output interface** - when the interface went away the kernel removed the route b4 had put in the set's table, the mark rule found the table empty, and the lookup fell through to the main table, so the set left through the ordinary uplink with the router's real address while its rules still read as correct. It arms only once the interface has existed, because a blackhole ends a route lookup rather than falling through, and arming one for an interface that had never appeared dropped every address the set matched inside the kernel. `Sets > Routing`.
...
- FIXED: **Importing a zapret configuration file produced one empty set out of a file holding a dozen strategies** - the reader worked a line at a time, so a value spanning several lines swallowed the rest of the file into a single unparsed argument, `$MODE_AUTO` and `%BIN%` were never resolved, `^` continuations survived as arguments, the right-hand side of `USER` and `TCP_PORTS` was read as options, nothing reproduced the order the launcher joins the separate TCP, QUIC and UDP variables in, and thirteen nfqws options that take an optional value were declared as requiring one, so their documented bare form ate the next argument, `--new` included.
- FIXED: **The import report described a set other than the one it created, and creating the sets took domains from the sets already working** - an option given twice was annotated only on its first occurrence so the value that applied read as unrecognised, a hex `--dpi-desync-fake-tls` was written into the fake payload as ASCII text under a row calling the option unsupported, every imported fake was given a TTL of 7 that nfqws never applies, an all-zero `--dpi-desync-fake-tls=0x00000000` was reported as having no equivalent although b4 carries an all-zero fake payload, a GoodbyeDPI command line was claimed as byedpi and a zapret2 one as zapret and both were converted anyway, and the domains an imported set claimed were removed from every enabled set that had them with nothing said.
