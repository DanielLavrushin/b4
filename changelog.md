...
- ADDED: **Blocking stats on the Dashboard** - when a set uses Block (blackhole) mode and actually blocks something, the Dashboard now shows a "Blackhole" panel with the total number of blocked attempts, the current rate per second, the most-blocked domains, and which devices ran into the most blocks. The panel stays hidden until there is something to show, so it never clutters the page when nothing is being blocked. Blocked connections are also tagged with a "block" label on the Traffic page so you can spot them in the live feed.
- FIXED: **Choosing specific devices was ignored for the Telegram bridge and other routed sets** - if you used Settings -> Devices to make b4 work for only some devices (or to exclude some), that choice was skipped for sets that route traffic, such as the Telegram (MTProto) bridge, an upstream proxy, or a Block set. Other devices on the network could still be sent through them. Your device choice now applies to these sets too. You can also limit an individual set to its own list of devices, so you can route or bridge traffic for just one device without affecting the rest.
