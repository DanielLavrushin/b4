...
- ADDED: **An MCP server at /api/mcp, so an external AI app such as Claude Desktop, LM Studio, Cursor or Jan can read b4's status, strategy sets, domain coverage, recent connections, log tail, metrics and diagnostics, and, with a separate opt-in, flip MTProto, SOCKS5, a set's enabled state or its fragmentation strategy** - working out why one site would not load meant reading the configuration, the connection list and the diagnostics bundle by hand and holding all three in your head at once, and a model asked to help had no way to see any of it. The model runs inside the external app, so b4 contacts no AI provider and needs no API key, which is what makes it usable on a headless router. Off by default, and it carries its own long-lived token because a web login token is discarded on every restart.
