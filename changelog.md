...
- FIXED: **A Discovery result for a CDN domain could not be added as a set when no geo database is installed** - the result carried geosite and geoip categories whatever was on the router, and a set naming a category with no database behind it is refused, so the attempt failed with nothing said beyond that the set could not be created.
- FIXED: **Restarting the service from the interface was recorded as a failed command on systemd** - b4 waited on a systemctl that systemd stops together with the service it is restarting, so every restart from the interface left `Restart command failed: signal: terminated` in the error log, and a restart that genuinely did not happen read exactly the same.
- FIXED: **A stop could spend b4's whole ten second window waiting on the web server, and left its firewall rules behind** - an unfinished browser connection held the shutdown open, and the unit b4 installs allowed exactly the same ten seconds, so the kill landed while the cleanup was still running.
- FIXED: **An update could overwrite a systemd unit with a SysV init script, after which the service stopped loading** - the updater recognised every init system except systemd, so a unit written by something other than b4's installer was rebuilt in the wrong format.
- FIXED: **An update could leave a second b4 running outside the service manager** - the installer waited one second for the service to come back and then started b4 by hand, which on systemd and procd races the manager's own restart until one of the two exits on the single-instance lock.
...
- FIXED: **A username without a password, or a password without a username, switched SOCKS5 authentication off rather than failing** - both fields had to be filled for the proxy to ask for credentials, so a half-finished pair quietly left the proxy accepting every client while the interface showed it as configured.
- FIXED: **The SOCKS5 UDP relay took the client's word for which address its datagrams would come from** - an address written into the associate request replaced the source of the control connection, so an association could be pointed at another host on the network and the relay served that host instead of the client.
...
- ADDED: **A UDP action that leaves matched packets alone** - every action a set could pick altered the traffic, so a set matching QUIC only to learn a site's addresses had to fake and fragment it anyway. `Sets > UDP`.
...
- ADDED: **A Block QUIC switch on a set's UDP tab** - pushing a browser off QUIC and back onto TCP took two settings in two places on the tab, with nothing saying they belonged together. `Sets > UDP`.
...
- FIXED: **A set bound to a manually added source device stopped taking its route** - such a device has no MAC address on the network, so its rule was built from a placeholder that matched no packet, and the same placeholder disarmed block sets and the all-devices-except-selected option.
- CHANGED: **The global device filter skipped devices added by hand** - ticking only those under Settings, Device Filtering left the filter inert and routing kept applying to every device on the network. Such devices count towards the routing half of the filter, matched by the IP address entered for them.
- FIXED: **An MSS clamp on a device added by hand clamped nothing and pulled every other forwarded connection down with it** - the rule matched no packet, and the catch-all covering the reply direction clamped every returning connection through the router to the smallest configured size.
...
- FIXED: **A routing set limited to source devices also diverted traffic the router itself sent** - the rules on the router's own outgoing traffic carried no device filter, and paired with Match any IP address that pulled its whole outbound path into the tunnel.
