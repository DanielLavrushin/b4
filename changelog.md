...
- FIXED: **The settings file was left open for any account on the router to read and change** - it holds the web interface login and the MCP access token, was created with open permissions and never corrected afterwards, and the copies kept before each upgrade carried the same permissions. b4 restricts the settings file and those copies at startup.
