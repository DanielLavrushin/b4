...
- FIXED: **In TUN mode with NAT Masquerade on and no interface selected, b4 captured traffic but no site opened through it** - the masquerade rule rewrote packets entering the TUN device to that device's own address before the TUN engine could put the uplink address on them, so b4 sent them out with a source the uplink could not answer.
