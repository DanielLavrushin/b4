...
- FIXED: **The diagnostics report showed no queue number, worker count or queue mode** - it read those three from config keys that b4 has never written, so the lines were silently skipped on every router.
- FIXED: **A failed start left part of b4's firewall rules on the router** - when applying the rules stopped partway, on a rejected rule or an unsupported kernel feature, the table, chains and hooks created up to that point stayed behind in the firewall after b4 had exited.
- FIXED: **A kernel without conntrack packet counters stopped b4 from starting at all** - the queue rules limit inspection to the first few packets of a connection using the kernel's per-connection packet counter, and where that counter is unavailable the whole rule was rejected and startup failed, even though b4 can run without the limit by inspecting every packet on the configured ports (a warning in the log records when it does).
- CHANGED: **System Info listed the wrong kernel modules on nftables-only routers** - the Kernel Modules section reported the iptables-era modules (xt_NFQUEUE, xt_connbytes) as missing on routers that use nftables only, such as OpenWRT 22 and newer, and said nothing about nft_queue and nft_ct, the modules b4 depends on there. A "Packet interception (NFQUEUE)" row sits alongside the list with the packages to install when interception is unavailable.
...
- FIXED: **The "already exists in other sets" warning missed overlaps that do change routing** - typing a domain into a set's Targets checks the other sets for a conflict, and that check compared entries character for character while traffic is matched by domain suffix. The check follows the same rules the matcher uses and spells out how the domain relates to the other set ("already in", "already matched by", "also takes over"), which entry is responsible, and whether that set is switched off; before that, `example.com` in another set drew no warning at all for `www.example.com` even though such traffic goes to the other set, regular-expression entries pulled in from geosite categories were compared as plain text and could never match, and entries belonging to switched-off sets were reported as if those sets were live. The API behind it, `GET /api/sets/check-domain`, carries the same detail per match and accepts several domains in one request. [#273](https://github.com/DanielLavrushin/b4/issues/273)
...
- FIXED: **Geosite and geoip files took far more memory than the categories selected from them** - reading a few categories out of a 51 MB file needed around 90 MB, enough to exhaust a router with little memory.
