...
- FIXED: **A routing set applied in TUN mode stopped taking effect after a firewall reload, and its addresses aged out of the set within the hour** - the loop that reinstalls routing rules and re-resolves a set's domains only ran on the NFQUEUE path, and a config save could not stand in for it because the sync skips any set whose rules it believes are already in place.
- FIXED: **Traffic a routing set claimed left by the TUN engine's uplink instead of the set's own route** - the engine's policy rules outranked the per-set ones and every packet it re-injected carried one fixed mark, so the set's interface, source address and NAT were all discarded on the way out. Traffic carrying a set's mark is left alone by the capture chain, the re-inject rule and the engine's source rewrite.
- FIXED: **Saving a set in TUN mode reported success and changed nothing until the service was restarted** - the capture rules were built once at start-up from the ports, packet limits and device list read at that moment, and no save path reached them.
- FIXED: **Discovery in TUN mode ran to completion and reported a verdict it had no way to measure** - it steers its probes through a second netfilter queue, which the TUN engine has no equivalent for, so every strategy tested the same unbypassed network and the run ended on run-to-run noise. Discovery and the watchdog's self-healing refuse to start on the TUN engine and say why.
- FIXED: **Blackhole sets and QUIC rejection were skipped for the traffic the TUN engine captured** - the engine put its own accept rule at the top of the forwarding chain, above the jumps those sets install, and a rejection sent to a forwarded client quoted the router's own address, which the client could not match to a connection.
- FIXED: **DNS interception in TUN mode dropped queries it could not answer and left the DNS-over-TCP redirect uninstalled** - the redirect lives in the rule set TUN mode never builds, and when the router cannot recover a forwarded client's address the intercepted query was dropped and its reply addressed to the uplink, leaving the client to retry against a listener with a 64-slot queue.
- ADDED: **A report of what the TUN engine and the routing rules are doing** - the engine reported a fixed "active" whatever state it was in, the firewall backend read as "none", and its own chains were filtered out of the diagnostics dump. Settings, System Info carries the policy rules, the routing tables and the engine's capture state, and the [Capture engine](https://docs.b4core.app/docs/settings/engine) page lists what TUN mode does not do.
