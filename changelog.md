...
FIXED: Telegram through the MTProto proxy dropped connections and loaded media slowly where Telegram's own servers are blocked or unreliable - b4 tried its routes one at a time and gave Telegram's edge three seconds before Cloudflare, longer than Telegram Desktop waits, retried a dead edge every five minutes and after a single lost handshake, and let its warm spares expire after 20 seconds.
