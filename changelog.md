...
- CHANGED: **The Discovery page is rebuilt around the sites typed into it** - results and history list the sites in the order given, with one card and a plain-words verdict per site, and a found strategy is described in a sentence and shown with the same split, fake, target and DNS rows as the Sets page before it is applied. A run shows its phase, the number of configurations tested and the live log line instead of a percentage, Stop keeps what was found and marks it unconfirmed, and the options are named by what they do.
