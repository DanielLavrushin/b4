...
- FIXED: **In TUN mode b4 stopped seeing any traffic after the router restarted its own firewall, for example when a port forward was changed on a Xiaomi router, until b4 itself was restarted** - b4 put back its jumps, NAT and forwarding rules but never checked the contents of its capture chain, so the chain stayed empty.
