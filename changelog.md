...
- FIXED: **Traffic to a Cloudflare Worker was cut into one small write per Telegram message** - that framing is what Telegram's own WebSocket edge requires, and it was applied to every WebSocket route, including a Worker, which does not need it: a Worker pours what it receives straight into a plain connection to Telegram.
- CHANGED: **Several Worker addresses are tried in a random order** - they were tried strictly as written, so the first one absorbed all the traffic and all the rate limiting while the rest sat unused.
