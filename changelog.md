...
- ADDED: **Sets can target whole networks by their ASN** - an ASN tab in the set editor takes a number such as `AS15169` or an IP address, b4 fetches the network's announced prefixes from RIPEstat into `asn_cache.json` next to the config and refreshes them daily, and the set stores only the number. The add dialog on the Traffic page offers the address, its announced prefix, a broader mask or the whole network, the DPI Detector adds a hosting network from its results, MCP `b4_edit_set_targets` takes the kind `asns`, and shared and community sets carry ASNs.
- FIXED: **Two overlapping packet duplication addresses on nftables removed every b4 rule and stopped the bypass, and turning duplication on or changing its addresses reached the firewall only with the next unrelated change** - the duplication address sets did not merge overlapping ranges such as `8.8.0.0/16` and `8.8.8.0/24`, so nft refused the whole rule load, and the check that decides whether a save needs a firewall refresh ignored the duplication addresses.
- FIXED: **A set whose GeoIP or GeoSite categories expanded to nothing, because a category was missing from the file or the file itself was gone, acted on every connection to its ports, and its MSS clamp on every connection from its source devices** - b4 took a set with no expanded domains or addresses for one that matches by port alone, and a per-set MSS clamp with no addresses fell back to the set's devices.
- FIXED: **After a configuration file was cut short, for example by a power loss during a save, b4 started on the defaults and wrote them over the file at the next save** - the file was rewritten in place, so an interrupted save left it truncated, and nothing kept a copy of a file b4 could not read before the next save replaced it.
- FIXED: **Adding an address from the Traffic page could undo a settings change saved at the same moment and showed a JSON parse error instead of the reason, and after it, as after a GeoIP or GeoSite file update, the MSS clamp and duplication addresses stayed stale until the next firewall refresh** - the add edited a copy of the configuration taken outside the save lock and answered errors as plain text, and neither it nor the file update refreshed the firewall sets that hold the expanded addresses.
- FIXED: **The IPInfo token could end up in b4's log, and an IPInfo or RIPE NCC lookup from the Traffic page could hang indefinitely** - the request carried the token in its URL, a failed connection logged that URL in full, and the lookups used a client with no timeout.
- FIXED: **Saving settings took seconds with large routing sets on nftables** - on every save b4 loaded a set's addresses 128 at a time with one `nft` call each, about 150 calls for 19,000 addresses.
...
- FIXED: **Telegram through the WebSocket bridge kept failing on networks that throttle workers.dev, with Passed on undecoded counting up on the Telegram card** - connections the bridge could not decode went to the Cloudflare Worker before a direct connection, even when the only Worker had been set aside after stalling and even for ports other than 443, which the Worker cannot reach.
- FIXED: **The Telegram bridge logged HTTPS connections to Telegram's web hosts as `invalid connection tag` errors, and b4's own download of the data-centre list went through the bridge and counted under Passed on undecoded** - the bridge recognised only the older 512-byte TLS ClientHello as HTTPS, and the download carried no mark, so the bridge's rule for the router's own connections took it in.
...
- ADDED: **Sets can process b4's own connections to the Cloudflare Worker** - with Let sets process Worker connections on under Settings, Telegram, a set with a working strategy for Cloudflare applies to the Worker connections, for networks that throttle workers.dev. It is off by default, and the Cloudflare-proxied domains skip DPI processing either way.
- FIXED: **b4 contacted core.telegram.org and raw.githubusercontent.com on every start with the MTProto proxy and the Telegram bridge off** - the Telegram data-centre list and the Cloudflare proxy domain list were downloaded whatever the settings, the second because its fallback is on by default.
...
- FIXED: **Photos and videos in some channels did not load on Android through Telegram over WebSocket, and the log filled with `-444` warnings** - the Android app leaves the data-centre field of its handshake random, so b4 took the data centre from the address, and 149.154.167.255 serves data centre 4's media from a block that otherwise belongs to data centre 2; each `-444` that followed also ranked the working routes down for every other data-centre-2 session.
- FIXED: **Proxy sets and Telegram over WebSocket stopped connecting after XrayUI restarted xray, while the Telegram card still read Working** - on every xray start XrayUI inserts a rule at the top of mangle PREROUTING that accepts every packet addressed to a local transparent socket, and it took the replies of connections b4 had diverted to its own listeners, so they stalled half-open.
- FIXED: **On iptables, the fake and split packets b4 sends for a set with an output interface went out through the main uplink instead of the set's interface after a settings save that rebuilt the firewall rules, such as a change of ports, packet limits, NAT Masquerade or an MSS clamp** - the rebuild put b4's rule that lets its own packets through back at the top of mangle OUTPUT, above the jumps into the sets' chains.
- FIXED: **A set with an output interface could delete every routing rule on the router and replace its default route when every routing table from 100 to 249 was already taken by other programs** - the sync skipped such a set, but a DNS answer for one of its domains installed it anyway with mark 0 and table 0, which `ip rule` and `ip route` read as every rule and the main table.
