...
- ADDED: **Import of a byedpi command line into sets** - a configuration that already worked under byedpi had to be rebuilt here by hand, option by option, and the two name almost nothing the same way, so the usual answer was to throw it away and start again from Discovery. b4 is an independent implementation rather than a fork of byedpi, so the translation is a best-effort one and says so: every option is reported as mapped, approximated, without an equivalent, or not applicable to a packet-level engine, and an option that does nothing in byedpi itself is called out rather than quietly carried over. Each byedpi profile becomes one set in the same order, and its `-A` retry chain becomes an escalation chain between them. A byedpi command line usually carries no host list at all, so domains are asked for during the import, per set rather than once for all of them: a set that only exists as the fallback of another one must stay target-less, or it competes with the set it is supposed to back up instead of taking over from it. A byedpi profile restricted to UDP is folded into the set that handles the rest of the traffic rather than becoming a set of its own, since a b4 set covers both protocols and the UDP one would otherwise claim the same domains first and let TCP through untouched. Sets page, Import.
- ADDED: **Import of a zapret (nfqws) command line into sets** - the same import understands zapret, including the `NFQWS_OPT="..."` form its config file uses, and the legacy `split`, `split2`, `disorder` and `disorder2` mode names alongside the current ones. zapret works on packets in the firewall the way b4 does, so more of it carries over than from byedpi: split positions, fooling methods, fake repeats, sequence overlap, packet duplication, port and address filters and the host list map directly. A zapret profile is an alternative selected by its filters rather than a retry step, so each becomes its own set with no escalation between them. Where several profiles share one host list and differ only by the port they filter, only the first can ever run: b4 picks one set per destination by domain and does not fall back to the next when the port does not match, which would have left the other ports with no handling at all and nothing said about it. The later sets are created disabled and named in the report.
