...
- ADDED: **Telegram's own WebSocket edge is reached under another TLS name where a DPI drops the handshake for its `kws*.web.telegram.org` names** - after such a handshake goes unanswered the warm pool tries the name `sprinthost.ru`, and client sessions use it once it has worked, as tg-ws-proxy does. The name is set, or fronting switched off with `off`, under Settings, MTProto Proxy, Fronting name for Telegram's WS edge.
