...
- ADDED: **Community sets: a set can be published to hub.b4core.app, and sets other b4 users published can be browsed, applied and rated from a new Community page** - a shared set carries only the strategy and its targets, leaves out and names everything tied to the router (routing except block mode, device filters, escalation, the DNS server address), carries its payload files inside, and is checked on the router when pasted into Import, where the plain export copied an enabled routing block verbatim, upstream credentials included, and a set naming a payload file the receiver lacked silently ran a built-in one; every publication waits for a moderator on the hub, the router keeps a signed copy of the catalogue and searches it by domain locally, through its own geosite categories as well, a set applied from the catalogue stays linked so it can be voted works or broken, updated when a newer version is approved, or reported, and Discovery tries up to three community sets for a domain before its own presets. All of it sits behind one switch under Settings, Integrations, off by default; a self-hosted hub or a child mirror of the central one is supported by the `b4hub` service that ships alongside.
...
- FIXED: **A passphrase-protected TLS key was accepted under Settings, and the web interface silently stayed on plain HTTP** - saving only checked that the two files existed, the failed load at start was a warning that never reached the error log or the dashboard, and a certificate file missing at start stopped the service altogether.
- CHANGED: **"Match Fake Size" copies the real ClientHello into the fake instead of stretching the payload to its size** - only the domain name is overwritten, with the payload's domain fitted to the real name's length, and Discovery tries this shape with an old TCP timestamp or a bad checksum at full TTL. Measured on two Russian networks, the filter drops a flow whose fake differs from the real greeting in length by a single byte and passes one of equal length, while the stretched fake repeated the payload's bytes and kept a stale handshake length, so it was no ClientHello at all; the DuckDuckGo preset was too large to leave the interface, and a "TCP Timestamp" fake reached the server intact when the client sent no timestamp option.
