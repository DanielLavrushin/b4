...
- FIXED: **The fakes and fragments b4 crafts for a routing set left by the router's ordinary uplink while the connection they belonged to went out the set's interface** - on nftables a proxy set put its own pair of mark returns at the top of the shared output chain, and that chain is a hook, so a return there ended it for every set hanging below. A set bound to particular devices lost the chain that hands those packets the set's routing mark altogether, because a filter written to match a LAN client can never match a packet the router originates itself.
...
- ADDED: **An MCP server that can act on b4 rather than only describe it** - the AI could read status and change one setting at a time, so anything a user asked for came back as instructions to carry out by hand, and a server able to rewrite strategy sets and make the router fetch a site left no trace of either. It gains per-setting facts, geosite and geoip search, target and set editing, the last update's transcript, a log line for every tool call and every request the endpoint turns away, and behind a separate **Allow active probes** permission, testing whether a domain loads and running Discovery. `Settings > Integrations`.
- FIXED: **A Discovery run's result became unreachable the moment the run ended, and the history meant to hold it grew by roughly half a megabyte per domain tested** - only the copy held in memory was consulted and that is dropped 30 seconds after a run finishes, so a strategy named while a run was still testing read as settled and stopping a run early threw away what it had found. Every strategy tried was saved with a full copy of the set b4 built to test it, so the hundred entries the file keeps could reach 60 MB on a router whose flash holds far less.
...
- FIXED: **On the TUN capture engine most of what b4 does outside the bypass itself quietly did not happen** - routing sets lost their route and were never repaired after a firewall reload, blackhole sets and QUIC rejection were skipped, the DNS-over-TCP redirect was never installed, and saving a set reported success without touching a single running rule. All of it is applied and kept alive on TUN the way it already was on NFQUEUE.
- FIXED: **Discovery on the TUN capture engine ran to the end and named a winning strategy it had no way to measure** - it steers its probes through a second netfilter queue that the TUN engine has no equivalent for, so every strategy tested the same unbypassed network and the winner came down to run-to-run noise. Discovery and the watchdog's self-healing refuse to start on TUN and say why.
- ADDED: **A report of what the TUN engine and the routing rules are doing** - the engine reported a fixed "active" whatever state it was in, the firewall backend read as "none", and its own chains were left out of the diagnostics dump. Settings, System Info carries the policy rules, the routing tables and the engine's capture state, and a new [Capture engine](https://docs.b4core.app/docs/settings/engine) page lists what TUN mode does not do.
